...
solver.logic = function(cube) {
	...
	// Check if cube is already solved before starting
	if (isCubeSolved(cube)) {
		typewriter.typeString("✅ Cube is already solved!").start();
		
		setTimeout(() => {
			if (window.showSolvedIndicator) {
				window.showSolvedIndicator();
			}
		}, 1000);
	...
	typewriter.typeString("🧩 Starting LBL solve...").pauseFor(1000).deleteAll().start();

	getCubeState();
	solveCube();

	function Execute(steps) {
		const originalDuration = cube.twistDuration;
		cube.twistDuration = 450;
		...
		function executeNextStep() {
			if (currentStep >= steps.length) {
				setTimeout(function() {
					cube.twistDuration = originalDuration;
					
					if (isCubeSolved(cube)) {
						typewriter.deleteAll().typeString("🎉 Cube solved successfully!").start();
						...
						typewriter.deleteAll().typeString("⚠️ Solve completed - please verify").start();
					}
				}, cube.twistDuration);
				return;
			}
			...
			if (step !== "") {
				if (currentStep !== steps.length - 1) {
					typewriter.typeString("🔄 " + stepName[currentStep] + "...")
						.pauseFor(500)
						.deleteAll()
						.start();
				} else {
					typewriter.typeString("🔄 Finalizing solve...").start();
				}
				
				const moves = step.match(/[UDLRFB][2']?|[udlrfb][2']?|[XYZ][2']?|[xyz][2']?/g) || [];
				let moveIndex = 0;
				
				function executeNextMove() {
					if (moveIndex < moves.length) {
						if (cube.isReady && cube.isReady()) {
							cube.twist(moves[moveIndex]);
							moveIndex++;
						}
						setTimeout(executeNextMove, cube.twistDuration * 0.8);
					} else {
						currentStep++;
						setTimeout(executeNextStep, cube.twistDuration);
					}
				}
				...
				if (moves.length > 0) {
					cube.twist(moves[0]);
					moveIndex++;
					setTimeout(executeNextMove, cube.twistDuration);
				} else {
					currentStep++;
				...
				if (currentStep !== steps.length - 1) {
					typewriter.typeString("✅ " + stepName[currentStep] + " already solved")
						.pauseFor(500)
						.deleteAll()
						.start();
				} else {
					typewriter.typeString("✅ " + stepName[currentStep] + " already solved").start();
				}
...
// Check if cube is solved - comprehensive validation
function isCubeSolved(cube) {
	if (!cube) return false;
	
	try {
		// Check each face has uniform color matching center piece
		const faces = ['front', 'up', 'right', 'down', 'left', 'back'];
		for (const face of faces) {
			const centerColor = cube[face].cubelets[4][face].color.name;
			for (let i = 0; i < 9; i++) {
				if (i === 4) continue; // Skip center
				if (cube[face].cubelets[i][face].color.name !== centerColor) {
					return false;
				}
			}
		}
		...
		console.error('Error checking solved state:', error);
		return false;
	}
}

console.log('Enhanced LBL solver with cube solved detection loaded successfully');